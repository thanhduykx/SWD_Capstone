...
| --- | --- | --- |
| Functionality / functional suitability | Business rules live in `CPMS.Core` and are covered by unit tests. Chairman-only session control, council membership, score locking and conflict checks must stay server-side. | `dotnet test .\CPMS.sln` |
...
| Maintainability | Layered architecture: API orchestration, Core rules/entities, Infrastructure persistence. No business rules hidden only in controllers or UI. | Code review, analyzer warnings |
...
- Keep domain decisions in `CPMS.Core`; controllers should coordinate requests, authorization and persistence.
